...
public class httpClient:@unchecked Sendable {
    ...
    public func post<T: Codable>(response: T.Type,url:String,data:Data,isSecure: Bool = false) async throws -> T {
        let response = try await load(response: response,url: url,method: .post,data: data,isSecure: isSecure)
        return response
    }
    public func get<T: Codable>(response: T.Type,url:String,data:Data? = nil,isSecure: Bool = false) async throws -> T {
        let response = try await load(response: response,url: url,method: .get,data: data,isSecure: isSecure)
        return response
    }
    public func put<T: Codable>(response: T.Type,url:String,data:Data,isSecure: Bool = false) async throws -> T {
        let response = try await load(response: response,url: url,method: .put,data: data,isSecure: isSecure)
        return response
    }
    public func patch<T: Codable>(response: T.Type,url:String,data:Data,isSecure: Bool = false) async throws -> T {
        let response = try await load(response: response,url: url,method: .patch,data: data,isSecure: isSecure)
        return response
    }
    public func delete<T: Codable>(response: T.Type,url:String,data:Data,isSecure: Bool = false) async throws -> T {
        let response = try await load(response: response,url: url,method: .delete,data: data,isSecure: isSecure)
        return response
    }
    
    private func load<T: Codable>(response: T.Type,url:String,method: httpMethod,data:Data?=nil,isSecure: Bool = false,token: String?=nil) async throws -> T {
